    ...
    while (writeBuffer.length > 0) {
      ...
      db.transaction(() => {
        for (const item of items) {
          ...
          db.run(
            `INSERT INTO requestDetails(id, timestamp, provider, model, connectionId, apiKeyId, status, data) VALUES(?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET timestamp = excluded.timestamp, provider = excluded.provider, model = excluded.model, connectionId = excluded.connectionId, apiKeyId = excluded.apiKeyId, status = excluded.status, data = excluded.data`,
            [record.id, record.timestamp, record.provider, record.model, record.connectionId, record.apiKeyId, record.status, stringifyJson(record)]
          );
        }
