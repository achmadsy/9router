...
export default function APIPageClient({ machineId }) {
 ...
 // Cloudflare Tunnel state
  ...
  const fetchData = async () => {
    try {
      // Endpoint page no longer manages keys — dedicated /dashboard/api-keys page.
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* Require API key — manage keys on dedicated page */}
      <Card id="require-api-key">
        <div className="flex items-center justify-between mb-2">
          ...
          <a
            ...
            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-border text-sm text-text-main hover:bg-surface"
          >
            Manage keys
        ...
        <p className="text-sm text-text-muted mb-4">
          Create, pause, reroll, and restrict per-key model/combo access on the dedicated API Keys page.
        </p>
        <p className="text-sm text-text-muted mb-2">
          {requireApiKey
            ? "Require API key is enabled — requests without a valid key are rejected."
            : "Require API key is currently disabled. Enable it on the API Keys page."}
        </p>
