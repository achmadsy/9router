...
export default function ZaiOAuthModal({ isOpen, providerInfo, onSuccess, onClose }) {
  ...
  const startPoll = useCallback(
    (id, initialDelayMs = POLL_INTERVAL_MS) => {
      stopPolling();
      setStep("polling");

      const schedule = (delayMs) => {
        pollTimerRef.current = setTimeout(runPoll, Math.max(1000, delayMs));
      };
      ...
      const runPoll = async () => {
        ...
        try {
          ...
          nextDelayMs = data.pollAfterMs || initialDelayMs;
  ...
  const startOAuth = useCallback(async () => {
    ...
    try {
      ...
      setStep("authorize");
      startPoll(data.flowId, data.pollAfterMs || POLL_INTERVAL_MS);
    } catch (err) {
      if (abortController.signal.aborted) return;
      setError(err.message);
      setStep("error");
        ...
        {step === "success" ? (
          ...
          <>
            <p className="text-sm text-text-muted">
              Sign in with your Z.AI account to connect a Coding Plan subscription. Authorization
              uses ZCode&apos;s server callback — no local redirect needed.
            </p>
