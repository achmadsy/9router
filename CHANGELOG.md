...
## Features
- **Self-Aware cooldown**: honour upstream wait headers (`Retry-After`, `x-ratelimit-reset`, …) before manual per-model wait settings, then fall back to the existing automatic cooldown path — precedence: upstream-header → manual-policy → provider-reset; new `/dashboard/self-aware` board shows every active cooldown (old + new mechanisms) with live countdown timers and per-row / bulk Reset; OpenCode Free cooldowns are isolated per proxy identity (proxy-pool id or `direct`), never fake connection rows or modelLock_*
- **Self-Aware cooldown (review fixes)**: await manual policy lookup so saved wait settings apply on 429; record OpenCode noauth proxy cooldowns from real `id: "noauth"` credentials shape (no fabricated `connectionId`); dashboard page uses cookie session like other pages (no phantom `@/hooks/useAuth`); propagate wait-header `cooldownHint` through STT/TTS/video/search/fetch/Gemini-native (429 and known quota-like statuses, incl. antigravity 409) — special TTS adapters (OpenAI, OpenRouter, Gemini, ElevenLabs, Xiaomi MiMo, MiniMax, self-hosted, Edge, Google) now throw typed errors carrying upstream status + cooldownHint, so TTS 429+Retry-After no longer degrades to 502/legacy-only cooldown; blank-model ("all") wait settings apply provider-wide when no exact model row (exact model still wins); dashboard reset clears only the selected account (exact connection id lookup, not the first row); board rows redact secrets/proxy URLs from legacy `lastError` when no sidecar exists; redact bearer tokens, API keys and proxy URLs in stored reasons; export/import `selfAwarePolicies` (older backups import as empty); composite PK `(provider, model)` for policies; clear account-cooldown sidecars on connection re-activation; page no longer trips react-hooks purity/set-state rules
- **Self-Aware cooldown (daily reset + labels)**: manual policy mode **Duration | Daily reset** — daily cools until the next local HH:MM (e.g. 00:00) instead of a fixed length; schema v5 adds `mode`/`resetHour`/`resetMinute`; board labels use registry display names and duplicate/custom node names (same as Add Duplicate modal); policy dropdown lists clones; clone-specific rows inherit base-provider policy when no exact clone row exists
