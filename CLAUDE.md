...
- Sentry error reporting: auto-initializes on import/bootstrap via `src/lib/sentry.js` if `SENTRY_DSN` is present. Errors and issue-keyword-matching warnings (`invalid`, `rate`, `limit`, `quota`, `captcha`, `token_refresh`, `re-auth`, etc.) route fail-open through `logger.js`, `consoleLogBuffer.js`, and `sentry.js` with rolling deduplication.
